 ...
 * @param filePath
 */
function default_1(filePath, jsFileArgs, onChange) {
    let resolvedPath = path.resolve(filePath);
    let extension = utils_1.getExtension(filePath);
    // Make sure the resolved file exists
    if (!extension) {
        if (fs.existsSync(resolvedPath + ".js"))
            extension = "js";
        else if (fs.existsSync(resolvedPath + ".yaml"))
            extension = "yaml";
        else if (fs.existsSync(resolvedPath + ".toml"))
            extension = "toml";
        else if (fs.existsSync(resolvedPath + ".json"))
            extension = "json";
        else if (fs.existsSync(resolvedPath + ".jsonc"))
            extension = "jsonc";
        else
            throw `Unable to resolve the extension for: ${filePath}`;
